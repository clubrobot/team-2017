...
class GotoBehavior:
	...
	def __call__(self, x_sp, y_sp, theta_sp):
		...
		x_in, y_in, theta_in = self.robot.wheeledbase.get_position()

		# Priori avoidance
		if self.brother_avoidance:
			for edge in self.robot.brother.get_edges():
		...
		if not self.brother_avoidance:
			path_not_found |= self.robot.brother.is_on_path(path)
		...
		if math.cos(math.atan2(path[1][1] - path[0][1], path[1][0] - path[0][0]) - theta_in) >= 0:
			...
			direction = -1
		self.robot.wheeledbase.lookahead.set(self.lookahead)
		self.robot.wheeledbase.lookaheadbis.set(self.lookahead)
		self.robot.wheeledbase.max_linvel.set(self.max_linvel)
		self.robot.wheeledbase.max_angvel.set(self.max_angvel)
		self.robot.wheeledbase.linpos_threshold.set(self.linpos_threshold)
		self.robot.wheeledbase.angpos_threshold.set(self.angpos_threshold)

		# Trajectory
		self.robot.wheeledbase.purepursuit(path, direction={1:'forward', -1:'backward'}[direction])
		...
		while not isarrived:
			...
			brother_distance = self.robot.brother.get_distance(x_in, y_in)
			if brother_distance < self.brother_distance:
				...
				if self.robot.brother.is_on_path(path):
					self.log('detected that brother is on the path')
					edges = self.robot.brother.get_edges()
					try:
						path = self.roadmap.get_shortest_path((x_in, y_in), (x_sp, y_sp))
						self.log('follow path: [{}]'.format(', '.join('({0[0]:.0f}, {0[1]:.0f})'.format(waypoint) for waypoint in path)))
						self.robot.wheeledbase.purepursuit(path, direction={1:'forward', -1:'backward'}[direction])
					...
					if path_not_found:
						...
						self.robot.wheeledbase.stop()
						return False

			# Get trajectory planner situation
			try:
				isarrived = self.robot.wheeledbase.isarrived()
			except RuntimeError:
			...
			if blocked:
				...
				self.robot.wheeledbase.set_velocities(-direction * 100, 0)
				time.sleep(1)
		...
		if theta_sp is not None:
			...
			self.robot.wheeledbase.turnonthespot(theta_sp)
			...
			while not isarrived:
				...
				if blocked:
					...
					self.robot.wheeledbase.set_velocities(0, -turndirection * 1.5)
					time.sleep(2)
					self.log('resume path')
					self.robot.wheeledbase.turnonthespot(theta_sp)
					blocked = False
